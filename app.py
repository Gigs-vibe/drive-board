...
class Api:
    ...
    def download_and_update(self, download_url):
        """Скачивает TaskaSetup.exe и запускает через Task Scheduler (вне любого job-объекта)."""
        ...
        # Bat-обёртка: schtasks /tr не любит кавычки внутри пути — проще передать .bat
        ...
        # Task Scheduler запускает bat вне job-объекта приложения — выживает после os._exit(0)
        run_time = (datetime.datetime.now() + datetime.timedelta(minutes=2)).strftime("%H:%M")
        try:
            r1 = subprocess.run(
                ["schtasks", "/create", "/tn", "TaskaAutoUpdate",
                 "/tr", wrapper, "/sc", "once", "/st", run_time, "/f"],
                creationflags=subprocess.CREATE_NO_WINDOW,
                capture_output=True, timeout=10,
            )
            if r1.returncode != 0:
                err = r1.stderr.decode("cp866", errors="replace").strip()
                return {"ok": False, "error": f"schtasks create: {err}"}
            r2 = subprocess.run(
                ["schtasks", "/run", "/tn", "TaskaAutoUpdate"],
                creationflags=subprocess.CREATE_NO_WINDOW,
                capture_output=True, timeout=10,
            )
            if r2.returncode != 0:
                err = r2.stderr.decode("cp866", errors="replace").strip()
                return {"ok": False, "error": f"schtasks run: {err}"}
        except Exception as ex:
            return {"ok": False, "error": f"Ошибка планировщика: {ex}"}
...
def get_asset_url(release_data):
    ...
    for asset in release_data.get("assets", []):
        if asset.get("name", "").lower() == "tasksetup.exe":
