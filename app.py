...
def data_dir() -> str:
    """Папка рядом с .exe (или со скриптом) — здесь хранятся данные доски."""
...
class Api:
    ...
    def download_and_update(self, download_url):
        """Скачивает Taska.zip, распаковывает, меняет папку приложения через PowerShell."""
        import subprocess, zipfile, shutil
        if not getattr(sys, "frozen", False):
            webbrowser.open(download_url)
            return {"ok": False, "error": "Запущено как скрипт"}

        current_exe = sys.executable            # .../Taska/Taska.exe
        app_dir = os.path.dirname(current_exe)  # .../Taska
        parent_dir = os.path.dirname(app_dir)   # родительская папка

        # staging в гарантированно ASCII-пути
        staging = r"C:\ProgramData\Taska"
        os.makedirs(staging, exist_ok=True)
        zip_path = os.path.join(staging, "Taska_update.zip")
        extract_dir = os.path.join(staging, "extract")
        ps_path = os.path.join(staging, "update.ps1")
        ...
        try:
            ...
            urllib.request.urlretrieve(download_url, zip_path, reporthook=on_progress)
        except Exception as ex:
        ...
        try:
            ...
            return {"ok": False, "error": f"Ошибка распаковки: {ex}"}

        new_app_dir = os.path.join(extract_dir, "Taska")
        old_dir = app_dir + "_old"

        # PowerShell-скрипт: меняет папки и копирует данные пользователя
        ps = "\n".join([
            "$old = '" + old_dir.replace("'", "''") + "'",
            "$cur = '" + app_dir.replace("'", "''") + "'",
            "$new = '" + new_app_dir.replace("'", "''") + "'",
            "$exe = '" + current_exe.replace("'", "''") + "'",
            "Start-Sleep 2",
            "if (Test-Path $old) { Remove-Item $old -Recurse -Force }",
            "Rename-Item $cur $old",
            "Move-Item $new $cur",
            # копируем данные пользователя из старой папки
            "foreach ($f in @('drive-board.json','drive-notified.json')) {",
            "  $src = Join-Path $old $f",
            "  if (Test-Path $src) { Copy-Item $src (Join-Path $cur $f) -Force }",
            "}",
            "Start-Process $exe",
            "Start-Sleep 4",
            "if (Test-Path $old) { Remove-Item $old -Recurse -Force }",
            "Remove-Item '" + ps_path.replace("'", "''") + "' -Force -ErrorAction SilentlyContinue",
        ])
        try:
...
def get_asset_url(release_data):
    """Возвращает прямую ссылку на Taska.zip из активов релиза."""
    for asset in release_data.get("assets", []):
        if asset.get("name", "").lower() == "taska.zip":
...
def show_update_toast(latest, page_url):
    ...
    try:
        ...
        t = Notification(app_id=APP_NAME, title="Доступно обновление",
                         msg=f"Вышла версия {latest} (у тебя {VERSION}). Открой приложение и нажми «Установить».",
                         icon=icon if os.path.exists(icon) else "")
        t.set_audio(audio.Default, loop=False)
        t.show()
    except Exception:
