...
class Api:
    ...
    def download_and_update(self, download_url):
        ...
        # Task Scheduler запускает установщик как независимый процесс Windows-сервиса —
        # гарантированно вне job-объекта приложения, выживает после os._exit(0)
        run_time = (datetime.datetime.now() + datetime.timedelta(minutes=2)).strftime("%H:%M")
        try:
            subprocess.run(
                ["schtasks", "/create", "/tn", "TaskaAutoUpdate",
                 "/tr", f'"{installer}" /VERYSILENT /NORESTART',
