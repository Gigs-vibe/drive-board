...
VERSION = "1.3"
# Заполни после создания репозитория, формат "логин/название". Пустая строка = проверка выключена.
GITHUB_REPO = "Gigs-vibe/drive-board"  # пример: "pssemenov/drive-board"
...
def show_update_toast(latest, url):
    if not HAVE_NOTIFY:
        return
    try:
        icon = resource_path("icon.png")
        t = Notification(app_id=APP_NAME, title="Доступно обновление",
                         msg=f"Вышла версия {latest} (у тебя {VERSION}). Нажми «Скачать».",
                         icon=icon if os.path.exists(icon) else "")
        try:
            t.add_actions(label="Скачать", launch=url)
        except Exception:
            pass
        t.set_audio(audio.Default, loop=False)
        t.show()
