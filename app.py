...
class Api:
    ...
    def download_and_update(self, download_url):
        """Скачивает новый Taska.exe, создаёт bat-скрипт для замены и перезапуска."""
        import subprocess
        ...
        new_exe = os.path.join(exe_dir, "Taska_update.exe")
        old_exe = os.path.join(exe_dir, "Taska_old.exe")
        bat_path = os.path.join(exe_dir, "taska_update.bat")
        try:
