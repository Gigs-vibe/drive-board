...
class Api:
    ...
    def download_and_update(self, download_url):
        """Скачивает новый Taska.exe и заменяет текущий через bat + cmd start."""
        import subprocess
        ...
        new_exe = os.path.join(staging, "Taska_new.exe")
        bat_path = os.path.join(staging, "update.bat")
        ...
        try:
            ...
            urllib.request.urlretrieve(download_url, new_exe, reporthook=on_progress)
        except Exception as ex:
            return {"ok": False, "error": f"Ошибка загрузки: {ex}"}

        # bat: ждём закрытия приложения, меняем exe, запускаем
        bat = (
            "@echo off\r\n"
            "chcp 65001 >nul 2>&1\r\n"
        ...
        try:
            # cmd /c start "" создаёт полностью независимый процесс —
            # он выживет после того, как мы закроемся через os._exit
            subprocess.Popen(
...
def get_asset_url(release_data):
    """Возвращает прямую ссылку на Taska.exe из активов релиза."""
    for asset in release_data.get("assets", []):
        if asset.get("name", "").lower() == "taska.exe":
