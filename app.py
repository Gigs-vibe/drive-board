...
class Api:
    ...
    def download_and_update(self, download_url):
        """Скачивает TaskaSetup.exe и запускает его тихо через ShellExecuteW (детач от job-объекта)."""
        import ctypes
        ...
        # ShellExecuteW создаёт процесс вне job-объекта текущего процесса —
        # installer выживет после os._exit(0), в отличие от subprocess.Popen
        try:
            ctypes.windll.shell32.ShellExecuteW(
                None, "open", installer, "/VERYSILENT /NORESTART", None, 1
            )
        except Exception as ex:
